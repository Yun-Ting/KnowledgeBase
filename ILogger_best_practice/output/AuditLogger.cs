...
public partial class AuditLogger : IAuditLogger
{
    ...
    private void Log(OperationResult result, string resource, string operationName, OperationType operationType, AuditLoggerType auditLoggerType)
    {
        ...
        try
        {
            if (auditLoggerType == AuditLoggerType.ControlPlane)
            {
                ...
                LogControlPlaneAuditSuccess(logger, operationName);
            }
            else
            {
                auditDataPlaneLogger.LogAudit(auditRecord);
                LogDataPlaneAuditSuccess(logger, operationName);
            }
        }
        catch (Exception ex)
        {
            LogAuditFailure(logger, auditLoggerType.ToString(), operationName, ex.StackTrace, ex);
        }
    }
    ...
    [LoggerMessage(Level = LogLevel.Information, Message = "Successfully pushed control plane audit record to geneva for operation {operationName}")]
    private static partial void LogControlPlaneAuditSuccess(ILogger logger, string operationName);

    [LoggerMessage(Level = LogLevel.Information, Message = "Successfully pushed data plane audit log to geneva for operation {operationName}")]
    private static partial void LogDataPlaneAuditSuccess(ILogger logger, string operationName);

    [LoggerMessage(Level = LogLevel.Error, Message = "Failed to push {auditLoggerType} audit record for operation {operationName} and trace is {stackTrace}")]
    private static partial void LogAuditFailure(ILogger logger, string auditLoggerType, string operationName, string stackTrace, Exception ex);
 }
